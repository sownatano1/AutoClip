...
YouTube → Whisper → análise do vídeo inteiro → perfil de conteúdo → duração inteligente → contexto/final → cenas/falantes → Visual Director → legendas/hook → capa → Cloudinary → Buffer → TikTok.

## Quality v9 — Visual Director

A v9 preserva os recursos anteriores e acrescenta três sistemas visuais:
...
- **Split-screen inteligente** — em `Podcast/Entrevista`, procura duas pessoas persistentes e separadas no quadro. Quando a segunda pessoa apresenta uma reação moderada e útil, mostra por aproximadamente 1.5–2.35 s o falante na metade superior e a reação na inferior. O uso é limitado por corte.
- **Reaction emphasis** — quando a segunda pessoa apresenta atividade facial mais forte, ou quando há sinais textuais de reação como risada/surpresa, o sistema pode fazer um close estático curto nela por aproximadamente 1.15 s e depois voltar ao falante.

Existe um **orçamento visual**: as intervenções precisam respeitar um intervalo mínimo e possuem limites por clip. Se o vídeo já estiver visualmente variado, a v9 pode simplesmente manter a edição existente.
...
- **Legendas** — tamanho 65, posição inferior, blocos curtos e destaque discreto em números/palavras importantes.
...
## Controles visuais da v9

No formulário **Run workflow** existem três controles novos, ativados por padrão:
...
Para Gameplay o sistema evita punch-ins artificiais do Visual Attention para não esconder HUD/ação. Split-screen é direcionado a Podcast/Entrevista.
...
4. Para avaliar v9, escolha `1` corte, Whisper `base`, legenda `English` e perfil `Podcast/Entrevista`.
...
O Job Summary mostra o vídeo, a capa, duração/final escolhidos e a descrição da edição visual aplicada (quantidade de punch-ins, split-screens e reações enfatizadas).
